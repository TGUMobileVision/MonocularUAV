...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub = nodeHandler.subscribe("/camera/color/image_raw", 1, &ImageGrabber::GrabImage,&igb);//D435i的图片话题
      //ros::Subscriber sub = nodeHandler.subscribe("/cam0/image_raw", 1, &ImageGrabber::GrabImage,&igb);//MH01的话题
    ros::spin();
...
void ImageGrabber::GrabImage(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    if (mpSLAMDATA->EnablePublishROSTopics())
    {
        ...
        mpSLAMDATA->PublishPoseForROS(cv_ptr);
           cout << "PublishPointCloudForROS" << endl;
